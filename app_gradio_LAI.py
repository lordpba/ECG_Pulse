...
def setup_llava_environment():
    ...
    if not llava_installed:
        ...
        try:
            ...
            print("LLaVA module found and loaded successfully.")
        except ImportError:
            # Add local LLaVA folder to PYTHONPATH if it exists
            ...
            if os.path.exists(llava_path):
                ...
                try:
                    import llava
                    llava_installed = True
                    print(f"LLaVA module loaded from local path: {llava_path}")
                except ImportError:
                    print(f"LLaVA found at {llava_path} but cannot be imported.")
                    llava_installed = False
            else:
                print("LLaVA module not found. Please install it or clone the repository.")
                print("Installation options:")
                print("1. Clone LLaVA repository: git clone https://github.com/haotian-liu/LLaVA.git")
                print("2. Install via pip: pip install llava (if available)")
                print("3. Or follow LLaVA installation instructions from their repository")
                llava_installed = False
...
def analyze_ecg(image, custom_prompt):
    ...
    """
    if not llava_installed:
        return """Error: LLaVA environment is not configured. 
        
Please install LLaVA by running one of the following:
1. git clone https://github.com/haotian-liu/LLaVA.git
2. pip install llava (if available)
3. Follow installation instructions from LLaVA repository
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return f"Error importing LLaVA modules: {import_error}. Please ensure LLaVA is properly installed."
        ...
        if "No module named 'llava'" in str(e):
            return """Error: LLaVA module not found. 

To fix this issue:
1. Clone the LLaVA repository: git clone https://github.com/haotian-liu/LLaVA.git
2. Install dependencies: pip install -e ./LLaVA
3. Restart the application

Alternatively, ensure LLaVA is installed in your Python environment."""
...
# Create the demo object at module level
